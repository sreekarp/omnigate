"""Token pricing and cost computation (in-process engine).

Prices are USD per 1,000 tokens, split into input (prompt) and output
(completion). Unknown models fall back to a zero price (cost 0) and log a
warning. Model-name resolution is forgiving: an ``azure/`` prefix is stripped
and dated/version suffixes are progressively trimmed so versioned ids price
correctly.

Ported from the gateway's ``app/services/pricing.py``; the SDK keeps its own
copy so it stays standalone (imports nothing from the server).

NOTE: the figures below are illustrative list prices and should be verified
against each provider's current pricing page before relying on them for billing.
"""
