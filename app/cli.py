"""``llm-gateway`` management CLI (stdlib argparse, no extra deps).
...
Subcommands::
    ...
    llm-gateway project create --org-id ID --name N [--daily-budget D] \\
...
def _cmd_version(_: argparse.Namespace) -> int:
    ...
    _print(f"llm-gateway {__version__}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="llm-gateway", description="LLM Gateway management CLI")
